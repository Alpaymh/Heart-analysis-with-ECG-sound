...
# AST modelini ve feature extractor'ı yükleme
...
def extract_features(file_list, augment=False):
    ...
    for file in file_list:
        ...
        # Audio spectrogram transformer requires log-mel spectrograms
        mel_spectrogram = librosa.feature.melspectrogram(y=audio, sr=16000, n_mels=128, fmax=8000)
        log_mel_spectrogram = librosa.power_to_db(mel_spectrogram)

        # Pencere boyutunu ayarlama
        log_mel_spectrogram = librosa.util.fix_length(log_mel_spectrogram, size=400, axis=1)
        
        inputs = feature_extractor(log_mel_spectrogram, return_tensors="pt", sampling_rate=16000, padding=True)
        with torch.no_grad():
